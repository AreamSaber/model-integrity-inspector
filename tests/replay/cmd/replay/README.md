...
The synthetic CLI fixtures are explicitly not real DB-settlement proofs or acceptance data. The separate [real Worker/TLS controller](../../capturefixture/README.md#b1-5-controlled-private-file-export-and-actual-cli) now exports private files only after actual final settlement, immutable publication, audit verification and decoded secret scanning. It builds and starts this same command after closing TLS and compares the analysis byte-for-byte with the real database publication on SQLite/PostgreSQL. That controller is test-only and uses an independent development Manifest signer, never an exported application master. No system firewall or global network configuration is changed here. Source/AST inspection or closing the mock server is not a substitute for actual OS isolation. Linux cross-compilation is not a claim that Linux filesystem tests executed on the Windows development host.
