...
func ForDialect(dialect string) ([]Migration, error) {
	...
	for i, name := range []string{"foundation", "queue_leases", "target_metadata", "identity_management", "target_prechecks", "catalog_versions", "execution_state", "response_evidence", "run_estimates", "execution_breaker", "estimate_expiry_index", "baseline_management", "review_receipts", "report_generation", "overview_current_targets", "evidence_display", "response_retention", "derived_attempt_source", "evidence_disclosures", "response_retention_cleanup", "system_maintenance", "backup_completion"} {
