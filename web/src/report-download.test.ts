...
function response(value: Report, body: BodyInit = bytes('{"safe":"S1"}'), change: Record<string, string> = {}) {
  return new Response(body, { headers: { 'Content-Type': `${value.format === 'json' ? 'application/json' : 'text/html'}; charset=utf-8`, 'Content-Disposition': `attachment; filename="report-${value.id}-r${value.revision}.${value.format}"`, 'Content-Length': String(value.file_size), 'X-Report-Content-Hash': value.content_hash!, 'X-Report-File-Hash': value.file_hash!, 'X-Content-Type-Options': 'nosniff', 'Content-Security-Policy': "sandbox; default-src 'none'; base-uri 'none'; form-action 'none'", ...change } })
}
...
describe('bounded verified report downloads', () => {
  it.each(['json', 'html'] as const)('verifies actual SHA-256 bytes and returns a non-executed %s Blob', async (format) => {
    const data = bytes(format === 'html' ? '<!doctype html><html><body>S1 中文</body></html>' : '{"safe":"S1 中文"}')
    const value = { ...record(data), format }, fetcher = fetchResponse(response(value, data))
    const file = await downloadReport(org, value)
    expect(file.blob.size).toBe(data.length); expect(file.blob.type).toBe(`${format === 'json' ? 'application/json' : 'text/html'}; charset=utf-8`)
