...
export function SessionLayout({ session, onSignedOut, onPasswordRequired }: { session: Session; onSignedOut: (notice: string) => void; onPasswordRequired: () => void }) {
  ...
  const trend = trendsRoute(route), validTrend = trend && trend.kind !== 'invalid'
  const activeRoute = historicalID ? 'runs' : resultID ? 'reports' : validComparison ? 'compare' : validTrend ? 'trends' : route
  const title = validTrend ? '单目标调用与风险趋势' : validComparison ? '两任务固定修订对比' : resultID ? '固定修订分析结果' : historicalID ? '检测进度与状态' : navigation.find(([key]) => key === route)?.[1] ?? '页面不存在'
