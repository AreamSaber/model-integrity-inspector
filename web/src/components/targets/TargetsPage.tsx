...
export function TargetsPage({ organizationID, csrfToken, userID, onSignedOut, onPasswordRequired }: TargetCallbacks & { userID: string }) {
  ...
  useEffect(() => {
    if (!editor && restoreListFocus.current && listHeading.current) {
  ...
  useEffect(() => {
    ...
    void targetsApi.list(organizationID, cursor, controller.signal, filters).then((result) => {
  ...
  async function remove(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!controller.signal.aborted) { restoreListFocus.current = true; setEditor(null); setNotice('目标及其当前加密凭证已删除；既有检测历史按服务端保留规则保留。'); refresh() }
    } catch (failure) { if (!controller.signal.aborted) handleError(failure) }
    finally { pending.current = false; if (!controller.signal.aborted) setBusy(false) }
  }
  ...
  return <>
      ...
      : editor?.mode === 'rotate'
        ...
        : editor?.mode === 'delete'
          ...
          : <section className="panel" aria-labelledby="target-list-title">
            <div className="section-heading"><h2 id="target-list-title" tabIndex={-1} ref={listHeading}>组织检测目标</h2><div className="form-actions"><button disabled={loading || busy} onClick={refresh}>刷新目标</button><button disabled={busy} onClick={() => { setEditor({ mode: 'create' }); setError(null); setNotice('') }}>新建目标</button></div></div>
            <p className="muted">列表和筛选来自当前组织的服务端查询。仅展示配置和凭证掩码，不返回 API Key、自定义请求头或密文。改变筛选会重置分页，不使用当前页数据冒充全量搜索。</p>
            <TargetFilters key={JSON.stringify(filters)} value={filters} disabled={loading || busy} onApply={(value) => { setFilters(value); setCursor(''); setHistory([]); setPage(null); refresh() }} />
    ...
    {!editor && <section className="panel"><div className="section-heading"><h2>正式检测</h2><span className="status-label">先预估，再确认费用</span></div><p className="muted">从目标行“配置检测”开始：读取当前有效权限、提交无上游调用的预估，确认费用后才创建真实 Run。需要当前目标版本通过预检；服务未就绪时会明确报错。检测历史与已有分析结果已接入；报告生成与导出尚未接入。</p></section>}
  </>
}
