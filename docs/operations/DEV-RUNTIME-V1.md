...
`internal/app/trends_pipeline_test.go` 在实际Quick18与Custom9的独立TLS检测后核对两Run、18/9调用分母、观测延迟、签名分页与筛选，并确保不把预检混入统计。SQLite/PostgreSQL三轮35.911s通过；这是完整测试耗时，不是性能验收。趋势前端及浏览器验证继续进行。
