...
后续实现状态更正（同日）：本说明是起点设计，不再代表当前代码实现数量。migration18、正常/重新认领/终态维护、分析读取与发布已接线；app已切换新估算的签名模式。root完成0/30天实际应用初始化→TLS→分析/报告/再次检测双库三轮79.678s，以及30天独立真实raw oracle等价、最终覆写与回滚、取消/失租/预算专项；最新证据见V1.0-持续开发台账、M5-DERIVED-APPLICATION-NOTES及各专项说明。混合并行回归有两次DATABASE_UNAVAILABLE，仍待串行复测，不能称最新全仓已绿。旧legacy正文策略、正文HTTP/UI及清理仍未完成。下文关于“未开始legacy可启用派生”的旧建议作废：来源模式现签入Manifest，所有已确认旧Run（包括尚未开始）保持原签名/模式，只有新估算确认才可选择可信部署模式；不做隐式重签或回填。
