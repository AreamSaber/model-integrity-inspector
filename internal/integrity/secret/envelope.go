...
type keySet struct{ wrap, fingerprint, audit, cursor, probe, evidence, baseline, display, derivedSource, requestReproduction, backupWrap []byte }
...
func NewKeyRing(active string, masters map[string][]byte) (*KeyRing, error) {
	...
	for version, master := range masters {
		...
		requestReproduction, err := derive("request-reproduction")
		if err != nil {
			return nil, ErrUnavailable
		}
		backupWrap, err := derive("backup-archive-wrap")
		if err != nil {
			return nil, ErrUnavailable
		}
		k.keys[version] = keySet{wrap: wrap, fingerprint: fingerprint, audit: audit, cursor: cursor, probe: probe, evidence: evidence, baseline: baseline, display: display, derivedSource: derivedSource, requestReproduction: requestReproduction, backupWrap: backupWrap}
	}
	return k, nil
}
