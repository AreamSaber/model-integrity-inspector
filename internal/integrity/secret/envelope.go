...
type keySet struct{ wrap, fingerprint, audit, cursor, probe, evidence, baseline, display, derivedSource, requestReproduction []byte }
...
func NewKeyRing(active string, masters map[string][]byte) (*KeyRing, error) {
	...
	for version, master := range masters {
		...
		k.keys[version] = keySet{wrap: wrap, fingerprint: fingerprint, audit: audit, cursor: cursor, probe: probe, evidence: evidence, baseline: baseline, display: display, derivedSource: derivedSource, requestReproduction: requestReproduction}
	}
	return k, nil
}
