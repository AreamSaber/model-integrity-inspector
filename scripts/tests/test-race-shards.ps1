...
$applicationPackage = 'model-integrity-inspector.local/mii/internal/app'
...
Test-MIIRaceCase 'one-parent execution does not create an empty second partition' {
    foreach ($group in @('Repository','Worker','Application')) {
...
foreach ($group in @('Repository','Worker','Application')) {
    foreach ($index in @(-1,6)) {
        Test-MIIRaceCase "$group out-of-range shard rejected by binding before execution" {
            $state = @{ Calls = 0 }; $failure = $null
            try { Invoke-MIICIRace -Group $group -Shard $index -Execute { $state.Calls++; throw 'Must not run' } } catch { $failure = $_ }
            if ($null -eq $failure -or $failure.Exception -isnot [System.Management.Automation.ParameterBindingException] -or $state.Calls -ne 0) { throw 'Invalid shard did not fail argument binding before execution.' }
        }
    }
}
$packages = @($applicationPackage, "$applicationPackage/extra", $package, "$package/extra", 'model-integrity-inspector.local/mii/scripts/tool', $workerPackage, "$workerPackage/extra")
Test-MIIRaceCase 'only exact repository package is partitioned out' {
    $rest = Get-MIINonRepositoryRacePackages -Packages $packages
    if ($rest.Count -ne 6 -or $rest -cnotcontains "$package/extra" -or $rest -ccontains $package) { throw 'A non-repository package was lost.' }
}
...
try {
    ...
    function Invoke-MIIMockedRace {
        ...
        $mockApplication = $applicationPackage
        $mockApplicationLines = $names + "ok  `t$applicationPackage`t0.123s"
        ...
        $execute = {
            ...
            $output = if ($GoArguments[0] -ceq 'list') { $mockPackages } elseif ($GoArguments -ccontains '-list') { if ($GoArguments[-1] -ceq $mockWorker) { $mockWorkerLines } elseif ($GoArguments[-1] -ceq $mockApplication) { $mockApplicationLines } else { $mockLines } } else { @() }
    ...
    Test-MIIRaceCase 'all non-repository packages and additional PostgreSQL identity/API pass' {
        ...
        if ($result.Failure -or $result.Calls.Count -ne 17) { throw "Complete regression did not run: $($result.Failure)" }
        $run = $result.Calls[3].Arguments
        if (($run[0..3] -join ',') -cne 'test,-race,-count=1,-timeout=10m' -or $run.Count -ne 8 -or $run -ccontains $package -or $run -ccontains $workerPackage -or $run -ccontains $applicationPackage) { throw 'Non-repository coverage or flags changed.' }
        foreach ($item in (Get-MIINonRepositoryRacePackages -Packages $packages)) { if ($item -cne $workerPackage -and $item -cne $applicationPackage -and $run -cnotcontains $item) { throw 'A package was omitted.' } }
        ...
        if (($result.Calls[2].Arguments -join ',') -cne "test,-race,-count=1,-timeout=10m,-list,^(Test|Example|Fuzz),$applicationPackage" -or -not $result.Calls[2].Capture) { throw 'Application must enumerate before any test body.' }
        foreach ($index in 0..5) {
            $part = $result.Calls[10 + $index]
            if ($part.Capture -or $part.Driver -or $part.Arguments.Count -ne 7 -or ($part.Arguments[0..4] -join ',') -cne 'test,-race,-count=1,-timeout=10m,-run' -or $part.Arguments[5] -cne (Get-MIIRacePattern -Names $plan.Shards[$index].Names) -or $part.Arguments[6] -cne $applicationPackage) { throw 'Application partition omitted a parent, widened timeout or changed database scope.' }
        }
        if (($result.Calls[16].Arguments -join ',') -cne 'test,-race,-count=1,-timeout=10m,./internal/identity,./internal/integrity/api' -or $result.Calls[16].Driver -cne 'postgres' -or $env:MII_IDENTITY_TEST_DRIVER) { throw 'PostgreSQL override or cleanup changed.' }
    }
    Test-MIIRaceCase 'Core retains every package except the three exact independently required packages' {
        ...
        $expected = @((Get-MIINonRepositoryRacePackages -Packages $packages) | Where-Object { $_ -cne $workerPackage -and $_ -cne $applicationPackage })
    ...
    foreach ($shard in 0..5) {
        Test-MIIRaceCase "independent Application shard $shard keeps exact complete parent selection" {
            $result = Invoke-MIIMockedRace -Group Application -Shard $shard
            if ($result.Failure -or $result.Calls.Count -ne 2) { throw 'Application shard must enumerate and execute once.' }
            if (($result.Calls[0].Arguments -join ',') -cne "test,-race,-count=1,-timeout=10m,-list,^(Test|Example|Fuzz),$applicationPackage" -or -not $result.Calls[0].Capture) { throw 'Standalone Application enumeration changed runnable kinds or flags.' }
            $plan = New-MIIRacePlan -Names $names
            $run = $result.Calls[1]
            if ($run.Capture -or $run.Driver -or $run.Arguments.Count -ne 7 -or ($run.Arguments[0..4] -join ',') -cne 'test,-race,-count=1,-timeout=10m,-run' -or $run.Arguments[5] -cne (Get-MIIRacePattern -Names $plan.Shards[$shard].Names) -or $run.Arguments[6] -cne $applicationPackage) { throw 'Standalone Application dropped a parent or changed its flags/database scope.' }
        }
    }
    ...
    Test-MIIRaceCase 'parallel CI delivery equals every local Other test invocation exactly once' {
        ...
        foreach ($shard in 0..5) { $separate += Invoke-MIIMockedRace -Group Application -Shard $shard }
        ...
        if ($expected.Count -ne 14 -or ($actual | ConvertTo-Json -Depth 5 -Compress) -cne ($expected | ConvertTo-Json -Depth 5 -Compress)) { throw 'Split CI omitted, duplicated or changed one of the complete Other test invocations.' }
    }
    foreach ($group in @('Other','Repository','Core','Worker','Application','IdentityPostgres')) {
        $steps = if ($group -ceq 'Other') { 17 } elseif ($group -ceq 'IdentityPostgres') { 1 } elseif ($group -ceq 'Repository') { 3 } else { 2 }
    ...
    foreach ($group in @('Core','Other','IdentityPostgres')) {
        Test-MIIRaceCase "$group rejects an accidental shard index before execution" {
            $state = @{ Calls = 0 }; $failure = $null
            try { Invoke-MIICIRace -Group $group -Shard 1 -Execute { $state.Calls++; throw 'Must not invoke' } } catch { $failure = $_ }
            if ($null -eq $failure -or $failure.Exception.Message -cne 'Only repository, Worker and Application support a shard index.' -or $state.Calls -ne 0) { throw 'Wrong non-sharded group failure or native command invoked.' }
        }
    }
    ...
    foreach ($mutation in @('missing-app','prefix-only','unknown-list','empty-parent','duplicate-parent')) {
        Test-MIIRaceCase "Application rejects $mutation before any test body" -MustFail -ErrorPattern 'exact Application package|Unknown or malformed race enumeration|Race enumeration requires tests|Duplicate enumerated race test' {
            $execute = {
                param([string[]]$GoArguments, [bool]$Capture)
                if ($GoArguments[0] -ceq 'list') {
                    $output = if ($mutation -ceq 'missing-app') { @($packages | Where-Object { $_ -cne $applicationPackage -and $_ -cne "$applicationPackage/extra" }) } elseif ($mutation -ceq 'prefix-only') { @($packages | Where-Object { $_ -cne $applicationPackage }) } else { $packages }
                } elseif ($GoArguments -ccontains '-list' -and $GoArguments[-1] -ceq $workerPackage) {
                    $output = $names + "ok`t$workerPackage`t0.1s"
                } elseif ($GoArguments -ccontains '-list' -and $GoArguments[-1] -ceq $applicationPackage) {
                    $output = switch ($mutation) {
                        'unknown-list' { @('unexpected-output', "ok`t$applicationPackage`t0.1s") }
                        'empty-parent' { @("ok`t$applicationPackage`t0.1s") }
                        'duplicate-parent' { $names + $names[0] + "ok`t$applicationPackage`t0.1s" }
                    }
                } else { throw 'Must reject enumeration before any race body.' }
                [PSCustomObject]@{ ExitCode = 0; Lines = $output }
            }
...
if ($NativeGo) {
    ...
    try {
        ...
        foreach ($selectedPackage in @($package, $workerPackage, $applicationPackage)) {
            ...
            foreach ($shard in $plan.Shards) {
                $group = if ($selectedPackage -ceq $package) { 'Repository' } elseif ($selectedPackage -ceq $workerPackage) { 'Worker' } else { 'Application' }
